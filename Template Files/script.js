...
function initCreative() {
	...
	var img = new Image();
	img.src =
		"https://bs.serving-sys.com/Serving/adServer.bs?cn=display&c=19&pli=1074204689&adid=1074740805&ord=" +
		rnd;
}
...
function onVideoEvent(event) {
	switch (event.type) {
		case "ended":
			...
			EB.automaticEventCounter("videoEnd2");
			EB.automaticEventCounter("videoEnd3");
			EB.automaticEventCounter("videoEnd4");
			EB.automaticEventCounter("videoEnd5");
			setTimeout(function() {
				VPAID.closeAd();
			}, 1000);
			break;
...
function setAudioImage() {
	if (VPAID.muted) {
		...
		clearTimeout(mutedTemp);
	} else {
		audioButton.style.backgroundImage = "url(images/audioOn.png)";
		if (!mutedTriggered) {
			clearTimeout(mutedTemp);
			mutedTemp = setTimeout(function() {
				EB.automaticEventCounter("unmuted_5seg");
			}, 5000);
		}
	}
}
